...
DEPENDS_${PN} = "bash"
RDEPENDS:${PN} = "bash"
SRC_URI:crimson = "file://fpga.rbf \
                   file://crimson-update.sh \
                   file://update.dtb \
                  "
SRC_URI:cyan = "file://fpga.rpd \
                file://cyan-update.sh \
                file://get_chipid_temperature.sh \
                file://jesd_rst_status.sh \
                file://test_ddr.sh \
               "
SRC_URI:chestnut =  "file://fpga.rpd \
					file://cyan-update.sh \
                    file://get_chipid_temperature.sh \
                    file://jesd_rst_status.sh \
                    file://test_ddr.sh \
                    "
FILES:${PN}:crimson += "${base_libdir}/firmware/"
FILES:${PN}:cyan += "${base_libdir}/firmware/ ${bindir}"
FILES:${PN}:chestnut += "${base_libdir}/firmware/ ${bindir}"
...
do_install:crimson() {
	...
	chown -R root:root ${D}${base_libdir}/firmware
